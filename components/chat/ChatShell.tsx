...
import { runTurn } from '@/lib/chat/client';
import { listConversations, loadConversation } from '@/lib/chat/load';
...
export function ChatShell() {
  ...
  const openConversation = useCallback(async (id: string) => {
    ...
    try { setMessages(await loadConversation(id)); } finally { setMessagesLoading(false); }
  }, []);
  ...
  }, [messages, liveSegments, liveText]);

  const runAndReload = useCallback(async (opts: Parameters<typeof runTurn>[0]) => {
    setSending(true);
    setTurnError(null);
    setLiveSegments([]);
    setLiveText('');
    let finishedConversationId: string | null = opts.conversationId ?? null;

    await runTurn(opts, {
      onConversationId: (id) => {
        finishedConversationId = id;
        if (!activeIdRef.current) { setActiveId(id); refreshConversations(); }
      },
      onTextDelta: (delta) => setLiveText((t) => t + delta),
      onHopComplete: (text) => {
        setLiveText('');
        if (text) setLiveSegments((segs) => [...segs, { type: 'text', text }]);
      },
      onToolStart: (toolUseId, toolName, args) => {
        setLiveSegments((segs) => [...segs, { type: 'tool', call: { key: toolUseId, tool_name: toolName, args, status: 'running' } }]);
      },
      onToolResult: (toolUseId, ok, result) => {
        setLiveSegments((segs) => segs.map((s) =>
          s.type === 'tool' && s.call.key === toolUseId ? { ...s, call: { ...s.call, status: ok ? 'done' : 'error', result } } : s
        ));
      },
      onDone: async () => {
        if (finishedConversationId) await openConversation(finishedConversationId);
        setPendingUserMessage(null);
        setLiveSegments([]);
        setLiveText('');
        setSending(false);
      },
      onError: (message, retryable, assistantMessageId) => {
        setTurnError({ message, retryable, assistantMessageId });
        setSending(false);
      },
    });
  ...
  const send = useCallback(() => {
    ...
    runAndReload({ conversationId: activeId ?? undefined, userMessage: text });
  }, [input, sending, activeId, runAndReload]);

  const retry = useCallback(() => {
    if (!turnError?.retryable || !turnError.assistantMessageId) return;
    runAndReload({ assistantMessageId: turnError.assistantMessageId, toolResults: [] });
  }, [turnError, runAndReload]);
  ...
  return (
    <div className="h-[calc(100vh-32px)] lg:h-[calc(100vh-56px)] -mt-1 grid grid-cols-1 lg:grid-cols-[220px_1fr_360px] gap-4 min-h-0">
      <div className="hidden lg:block min-h-0">
        <ThreadsSidebar conversations={conversations} activeId={activeId} onSelect={openConversation} onNew={startNew} loading={conversationsLoading} />
      </div>
      ...
      <div className="bg-white border border-[var(--line)] rounded-xl flex flex-col min-h-0">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
          ...
          {!messagesLoading && messages.length === 0 && !pendingUserMessage && (
          ...
          {(sending || liveSegments.length > 0 || liveText) && (
            <MessageBubble role="assistant">
              <div className="space-y-2">
                {liveSegments.map((seg, i) =>
                  seg.type === 'tool'
                    ? <ToolCallChip key={i} call={seg.call} />
                    : <div key={i} className="text-[13px] text-[var(--muted)] italic whitespace-pre-wrap">{seg.text}</div>
                )}
                {liveText && <div className="whitespace-pre-wrap">{liveText}</div>}
                {sending && !liveText && liveSegments.length === 0 && <ThinkingDots />}
              </div>
            </MessageBubble>
          )}
