...
⚠️ **CORRECTED (BUILD.md Phase 2, 2026-09-09)** — see §12's `max_rows` footgun. This
section previously said single-page fetching "is acceptable for current upload sizes";
that was wrong for `mna` and silently corrupted every `mna_pct`/`mna_graneles_pct`/
`mna_carnes_pct`/`mna_fyv_pct` snapshot ever computed. Rows are now fetched **one upload
at a time, PAGED within each upload** (`.range()` looped until an under-full page comes
back) using a simple `eq('upload_id', u.id)`.

Do NOT restore `ORDER BY id` + range pagination **across uploads** — that triggers a full
table sort on a growing `upload_rows` table and causes Supabase statement timeouts on
weeks with many uploads (25+). Paging *within* one upload_id's own rows has no such cost —
it's still a single-value index scan, just repeated.
...
| **`max_rows` silently truncates `.limit()`/`.range()` past 1000 — NOT just unranged queries (BUILD.md Phase 2, session 15)** | This project's PostgREST "Max Rows" project setting is 1000, and it caps *any* request for more, including an explicit `.limit(10_000)` or `.range(0, 49999)` — no error, no warning, just silently fewer rows back. §9's `upload_rows` fetch strategy note previously said single-page `.limit(10_000)` fetching "is acceptable for current upload sizes" — **that was wrong.** Verified against a real `mna` upload: 5,007 true rows (exact count), `.limit(10_000)` returned exactly 1000. `mna` is the only app whose uploads exceed 1000 rows (~4,800-5,000/hub-week; every other app checked stays well under it) — but this had been **silently corrupting every `mna_pct`/`mna_graneles_pct`/`mna_carnes_pct`/`mna_fyv_pct` snapshot ever computed** (`lib/kpi-compute.ts`) and the live MNA breakdown tile (`historicos/page.tsx`, ~80% of SKUs invisible to any top-merma ranking) since those features shipped. Both fixed to page with `.range()` per upload_id until an under-full page returns; `lib/analysis/*`'s own `fetchRowsForUploads` does the same. All 23 affected weeks recomputed (`scripts/recompute-mna-weeks.ts`) — spot-checked `mh_contry`/`2026-04-17`: `mna_pct` corrected from 2.61% to 2.92%. **Any future query that might return more than 1000 rows MUST paginate — a bigger `.limit()` number is not a fix, it fails silently instead of erroring, which is worse than a hard cap that throws.** |
...
## 24. Chat assistant — future v edit (session 15, 2026-09-10, not built)

This is a roadmap note, not a footgun — nothing here is broken, and nothing described below has been implemented. Written after live-testing the chat feature built this session (BUILD.md Phases 0–4, see §§ above / README footnote) against a real out-of-scope question to characterize what it actually is.

### Finding: it's a narrow specialist, not a general assistant

Tested live at `/chat` by asking it something with zero connection to the ops data (a hamburger recipe). It correctly declined to fabricate an answer, stayed polite, and redirected toward what it can actually do — it did not hallucinate a recipe dressed up as a "finding." That's the right failure mode for a tool whose entire trust model is claims-with-evidence (`lib/analysis/evidence.ts`, the `claim:` citation links in `MessageBubble.tsx`): every claim it makes is backed by a re-runnable query against `upload_rows`/`order_deliveries`, and a hamburger recipe has no such backing, so it has nothing to cite and (correctly) won't assert one.

But this confirms the assistant is a **narrow Calii-ops investigation specialist by design**, not a Claude-Cowork-style general assistant that happens to know about Calii. The tool registry (`lib/analysis/tool-registry.ts`, 16 tools) and system prompt (`lib/prompts/chat-system-prompt.ts`) only know how to query and analyze the weekly-upload data — there is no general reasoning/knowledge/task-execution surface exposed to it beyond that.

### Wishlist (Jose, this session): web browsing, email drafting, presentation creation, CSV export

None of these exist today. Rough shape of what each would take, for scoping a future session — not estimates to commit to:

- **Web browsing** — would need a new tool (e.g. `webSearch`/`webFetch`) added to the tool registry and exposed in the system prompt. Straightforward mechanically (Anthropic's web tools or a custom fetch+parse tool), but breaks the claims/evidence model as built: a claim's `evidence.refetch` currently assumes a deterministic re-runnable query against our own DB — a live web page can change or disappear, so citations from browsing would need a different, weaker provenance story (e.g. "as of {timestamp}, snapshot below" rather than "re-run this to verify").
- **Email drafting** — needs an email-send/draft tool wired to whatever mail provider, plus (per this session's action-category rules) should almost certainly stop short of actually sending — draft-and-hand-to-Jose, not autosend. Isolated addition, doesn't touch the analysis core.
- **Presentation / CSV creation** — needs an output-artifact tool (e.g. generate a file and offer it for download) that doesn't exist in this app yet; today the chat only renders markdown + inline claim citations, there's no file-generation or download surface in `ChatShell.tsx`/`ExhibitionPane.tsx`.

All four are additive — new tools alongside the existing 16, not changes to the analysis core — so they don't conflict with anything built this session. None started; not scheduled.

### Wishlist item 5 (Jose, this session): ad-hoc file upload from inside a chat investigation

Raised in this and past sessions: the 5 registered weekly apps (20 CSVs/week → `upload_rows`) aren't the only data Calii has. If Jose is mid-investigation on, say, faltantes and wants to bring in `registro de inventario` or whatever other file seems relevant *in that moment*, he currently can't — the only upload path is `/upload`, and it only accepts the pre-registered weekly apps in the `apps` table (`app_id` is validated against it, see `app/api/upload/route.ts:69-74` — unknown `app_id` is rejected outright).

This is a different shape of problem than "register a new recurring weekly app," which the README already documents as a no-code UI flow (Configuración → Apps & KPIs). That flow assumes the file recurs every week, needs a week/city/hub slot, and gets a permanent KPI/column schema defined up front — too heavyweight for "let me hand you one file, right now, just for this conversation." What's missing is a *lighter*, session-scoped path: attach a file to the chat, have it parsed and made queryable by the model for that investigation, without necessarily going through week-scoping, identity validation (`lib/validate.ts`'s IDENTITY_CONFIG checks — see §22), or a formal `apps`/`app_columns` registration.

Rough shape for a future build, not designed yet:
- A file-attach affordance in `ChatShell.tsx` (there's currently no file input at all, only the textarea) + a new `/api/chat/upload`-style endpoint that stores the raw file (Supabase Storage, no `apps` row required) and parses it into a queryable shape (temp table, or just loaded into memory and handed to the model as a large tool result, depending on size).
- A new tool (e.g. `queryUploadedFile`) exposed to the model for that turn/conversation so it can actually analyze the ad-hoc data — the existing 16 tools in `lib/analysis/tool-registry.ts` are all hardcoded against known tables (`upload_rows`, `order_deliveries`, etc.) and know nothing about a shape they've never seen.
- A decision on scope/lifetime: is the file conversation-scoped only (gone/orphaned once the thread ends), or promotable to a real registered app later if it turns out to be useful every week? Probably the latter should be a manual "promote this to a real app" action, not automatic.
- Claims/evidence implications: same concern as compaction below — if the model cites something from the ad-hoc file, the citation's `evidence.refetch` needs a real re-runnable query, which means even a "quick" ad-hoc upload can't skip having *some* durable, queryable storage — it can skip the week/city/hub/KPI scaffolding, but not a persisted, addressable table.

Not started. Documentation only, per explicit instruction.

### Context compaction — direct answers to Jose's questions

- **Has it been revised since we discussed it?** No. Nothing has changed — it was discussed (in comparison to how Claude Cowork/long-running sessions handle it) but never designed or built this session. `reconstructMessages()` in `app/api/chat/route.ts` still does the simple thing: every turn replays the *entire* message history for that conversation, uncompacted, straight from the `messages`/`tool_calls` tables.
- **Is it an easy add?** No — harder than it looks, for a reason specific to this app's design. A generic chat app can summarize old turns away with only quality loss. Here, every claim the model has made is keyed to a `tool_call_id` row (`claims.tool_call_id` → `tool_calls.result`), and the `claim:` links in the UI (`MessageBubble.tsx`, `ExhibitionPane.tsx`) depend on that full tool result still being resolvable so the exhibition pane can render it. Naive compaction (summarize old turns to text, drop the raw tool_use/tool_result blocks) would silently break every citation from a compacted-away turn — the evidence link would point at data that's no longer in the replayed context, or worse, no longer reconstructable at all if the summary replaces rather than supplements the stored rows. A real implementation needs: (1) a trigger (token-count threshold on the reconstructed message array), (2) a compaction step that summarizes old assistant turns for the *model's* context while leaving the underlying `tool_calls`/`claims` rows in the DB untouched so old citations still resolve from history even though the model itself no longer sees that content, and (3) UI handling for "this claim is from a compacted turn" so Jose isn't confused when an old citation still works but the model itself can no longer recall the detail if asked a follow-up.
- **What would I sacrifice if we do it?** Evidence/citation fidelity for older parts of a long conversation is the main one — the DB rows stay correct and re-runnable, but the model's own working memory of *why* it made an old claim degrades to a summary, so a follow-up like "why did you say X three turns ago" gets a weaker answer. Also: added implementation + testing surface (this is exactly the kind of change that needs the same live-DB-backed verification rigor as the Max Rows fix, not just a typecheck), and there's a real chance it's premature — the cost-per-response note added this session (§ ChatShell footer) already nudges Jose toward starting a fresh conversation per question rather than building up the long threads where compaction would actually start to matter. Worth revisiting once real usage shows conversations regularly running long enough for it to bite, not before.

**Status: none of this is built. Documentation/analysis only, per explicit instruction — do not implement without a separate go-ahead.**

## 25. /historicos production outage — MNA pagination round-trip fix (session 15, 2026-09-10)

**What happened:** the MNA Max Rows correctness fix earlier this session (§12's addendum, `mnaRawPages` in `app/(app)/historicos/page.tsx`) replaced one fast-but-wrong `.limit(10_000)` query per MNA upload with a correct but SEQUENTIAL per-upload pagination loop (5 round trips for a ~4,800-row upload). That broke `/historicos` on Netlify entirely — José reported it had already been intermittently failing before this session ("used to happen sometimes, a reload would fix it"), and this fix pushed it to failing consistently.

**Confirmed root cause, not guessed:** Netlify Observability on the failing request showed `200 OK`, ~18s duration, ~1KB response body. A real rendered `/historicos` payload is much larger than 1KB — a 200 with a near-empty body after that long is a truncated RSC stream, not a successful render. The browser's own error — `Error: Connection closed.`, thrown from the shared framework chunk during RSC decode — is the client-side signature of exactly that: the host cut the connection mid-stream, after the 200 header had already gone out (so it can't downgrade to a clean error status). This is consistent with a host-side response/function time limit, not a bug in the render logic itself — locally, in both `next dev` and a real `next build && next start`, against the same hosted Supabase project (confirmed via `.env.local`), the page always completed successfully, just slowly.

**Fix:** in the same file, (1) the MNA branch now does one `head: true` count query per upload (parallel) then fires every page's `range()` query for every upload in one flat `Promise.all` — 2 round trips instead of 5 sequential ones — and (2) a small shared `createLimiter(8)` gate now wraps every one of this page's 60+ independent Supabase queries (the 13-query counts/registry batch in Step 2, and every paginated branch in Step 4), capping peak concurrent requests instead of letting a bare `Promise.all` burst unbounded. Both are real, structural reductions in round-trip count / connection burst, not tuning.

**⚠️ Caveat on verification:** this session's local dev environment had ~3s of fixed per-connection overhead to Supabase (confirmed via a bare `curl` to the Supabase REST endpoint — TCP+TLS alone took ~2.9s), which is a problem with that testing machine's network path, not the app. That inflated every local timing measurement and made this session's own "before/after" numbers unreliable as a proxy for Netlify's actual latency budget — the structural fix (fewer round trips, bounded burst) is sound regardless, but **the actual margin this buys on Netlify was not independently confirmed from this session**. Check Netlify Observability on the next `/historicos` request after deploy — look for status 200 with a normal-sized response body (not ~1KB) and a duration that isn't suspiciously close to whatever cut it off at ~18s before.

**If this fix doesn't fully resolve it**, the next lever is raising Supabase's PostgREST "Max Rows" setting (Project Settings → API — José couldn't locate this field in the dashboard as of this session; may be under a different label, or gated by plan) so a single request can return more than 1000 rows, cutting the MNA branch to 1 round trip per upload instead of 2. A Netlify rollback to the pre-session deploy (Deploys → previous deploy → "Publish deploy") remains available as an immediate, code-independent mitigation if uptime is more urgent than root-causing further.

### ⚠️ UPDATE, same session, after deploying the fix above: it did NOT help — round-trip count / concurrency was the WRONG theory

José pushed the fix, confirmed the Netlify deploy, and re-tested. Netlify Observability on the new request: **duration 17879ms** — essentially identical to the pre-fix **17975ms** (well within noise, ~0.5% different). This is the important finding of this session, more important than the fix itself:

**Two structurally different versions of this page — one doing 5 sequential MNA round trips with unbounded concurrency, one doing 2 MNA round trips with concurrency capped at 8 — landed on the exact same ~18s wall-clock time.** If the bottleneck were round-trip count or connection-burst scheduling, changing both of those substantially should have moved the number. It didn't, at all. That rules out this session's theory: **this is not a client-side request-scheduling problem.**

What the ~18s-regardless-of-restructuring pattern actually suggests: a **fixed timeout** (Netlify function/edge response limit, or possibly a Postgres `statement_timeout` on Supabase's side) that this page's total work still exceeds no matter how the requests are batched or ordered — the page keeps getting cut at the same wall-clock point because something outside the request-scheduling logic is enforcing a ceiling there, not because that's how long the work "naturally" takes.

**What wasn't tried, and is the more promising direction for next session:** this page recomputes the MNA and faltantes aggregations from thousands of raw `upload_rows` on *every single page load* (the `allMnaRows`/`allFaltantesRows` → `mnaProducts`/`faltantesSkuProducts` aggregation logic in Steps 5–6 of `historicos/page.tsx`). That's real, uncached, repeated work against real data volume on every request. This app already has a working precedent for NOT doing that: `kpi_snapshots` are computed once (via `lib/kpi-compute.ts`, triggered from upload/recompute) and the page just reads the precomputed table — fast, no raw-row aggregation at request time. The MNA/faltantes tile-flip aggregation was never moved onto that pattern; it's the one part of this page still doing heavy live computation per request. Precomputing it (write a `mna_products`/`faltantes_sku_products`-style table or extend `kpi-compute.ts`, populate at upload/recompute time, have the page just read it) would cut both data volume and query count dramatically, unlike this session's fix which only changed *how* the same volume of raw-row fetching was scheduled.

**Also worth checking before diving into a rebuild:** whether Supabase has a configurable `statement_timeout` (Project Settings, or via `ALTER ROLE ... SET statement_timeout`) that's shorter than expected for this project's plan, and what Netlify's actual function/response timeout is for this site (may require contacting Netlify support or checking plan docs — this wasn't determined this session, only inferred from behavior).
